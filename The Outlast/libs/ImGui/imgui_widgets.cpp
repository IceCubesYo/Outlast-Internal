...
bool ImGui::tab(const char* label, const char* icon, bool selected)
{
    ...
    if (selected)
    {
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(72, 79, 89));
        window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 3, bb.Max.y), ImColor(215, 219, 228));
    }

    window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Max.y), bb.Max, ImColor(46, 52, 64));
    window->DrawList->AddLine(ImVec2(bb.Min.x + 60, bb.Min.y), ImVec2(bb.Min.x + 60, bb.Max.y), ImColor(46, 52, 64));

    ImGui::PushFont(iconfont);
    ImGui::RenderText(ImVec2(bb.Min.x + 30 - icon_size.x / 2, bb.Min.y + (size.y / 2) - (icon_size.y / 2)), icon);
    ImGui::PopFont();
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    draw->AddRectFilledMultiColorRounded(bb.Min, bb.Max, ImColor(76, 103, 213), ImColor(124, 57, 169), ImColor(124, 57, 169), ImColor(76, 103, 213), g.Style.Rounding, 15);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Min.x + 25, check_bb.Min.y - 1), label);
    ImGui::PopStyleColor();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x, frame_bb.Max.y), ImColor(141, 142, 144), style.ComboRounding, popup_open ? 3 : 15);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(160, 13));
    const ImRect total_bb(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 20), ImVec2(frame_bb.Max.x + 5, frame_bb.Max.y));
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 4), ImVec2(frame_bb.Max.x, frame_bb.Max.y - 4), ImColor(36, 40, 49), 9);
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 4), ImVec2(grab_bb.Min.x, frame_bb.Max.y - 4), ImColor(215, 219, 228), 9);

    // Render grab
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 1, grab_bb.Min.y + 24), 5, ImColor(255, 255, 255), 360);
    ...
    if (label_size.x > 0.0f)
    {
        draw->AddText(ImVec2(frame_bb.Min.x + 5, frame_bb.Min.y - 12), ImColor(255, 255, 255), label);
    }

    if (hovered || ImGui::IsItemActive())
    {
        draw->AddRectFilled(ImVec2(grab_bb.Min.x - 15, total_bb.Min.y + 5), ImVec2(grab_bb.Max.x + 10, total_bb.Min.y + 20), ImColor(34, 37, 42), 4);
        RenderTextClipped(ImVec2(grab_bb.Min.x - 15, total_bb.Min.y + 5), ImVec2(grab_bb.Max.x + 10, total_bb.Min.y + 20), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), 24, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            ...
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 24, ImDrawCornerFlags_All);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), 24); // Color button are often in need of some sort of border
    }
...
bool ImGui::sub(const char* label, bool selected)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(150, 60), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    if (selected) {
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(72, 79, 89));
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 3), ImVec2(bb.Max.x, bb.Max.y), ImColor(215, 219, 228));
    }
