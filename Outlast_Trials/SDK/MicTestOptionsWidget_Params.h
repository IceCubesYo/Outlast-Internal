#pragma once

/**
 * Name: Outlast_Trials
 * Version: 1.3
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function MicTestOptionsWidget.MicTestOptionsWidget_C.GetTestButtonText
	 */
	struct UMicTestOptionsWidget_C_GetTestButtonText_Params
	{
	public:
		class FText                                                ReturnValue;                                             // 0x0000(0x0018)  (Parm, OutParm, ReturnParm)
	};

	/**
	 * Function MicTestOptionsWidget.MicTestOptionsWidget_C.BndEvt__MicTestOptionsWidget_MicTestButton_K2Node_ComponentBoundEvent_4_OnButtonClickedEvent__DelegateSignature
	 */
	struct UMicTestOptionsWidget_C_BndEvt__MicTestOptionsWidget_MicTestButton_K2Node_ComponentBoundEvent_4_OnButtonClickedEvent__DelegateSignature_Params
	{	};

	/**
	 * Function MicTestOptionsWidget.MicTestOptionsWidget_C.Tick
	 */
	struct UMicTestOptionsWidget_C_Tick_Params
	{
	public:
		struct FGeometry                                           MyGeometry;                                              // 0x0000(0x0038)  (BlueprintVisible, BlueprintReadOnly, Parm, IsPlainOldData, NoDestructor)
		float                                                      InDeltaTime;                                             // 0x0038(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

	/**
	 * Function MicTestOptionsWidget.MicTestOptionsWidget_C.BndEvt__MicTestOptionsWidget_Button_1_K2Node_ComponentBoundEvent_0_OnButtonHoverEvent__DelegateSignature
	 */
	struct UMicTestOptionsWidget_C_BndEvt__MicTestOptionsWidget_Button_1_K2Node_ComponentBoundEvent_0_OnButtonHoverEvent__DelegateSignature_Params
	{	};

	/**
	 * Function MicTestOptionsWidget.MicTestOptionsWidget_C.HighlightEntry
	 */
	struct UMicTestOptionsWidget_C_HighlightEntry_Params
	{	};

	/**
	 * Function MicTestOptionsWidget.MicTestOptionsWidget_C.DownplayEntry
	 */
	struct UMicTestOptionsWidget_C_DownplayEntry_Params
	{	};

	/**
	 * Function MicTestOptionsWidget.MicTestOptionsWidget_C.OnEchoChannelStateChanged
	 */
	struct UMicTestOptionsWidget_C_OnEchoChannelStateChanged_Params
	{	};

	/**
	 * Function MicTestOptionsWidget.MicTestOptionsWidget_C.ExecuteUbergraph_MicTestOptionsWidget
	 */
	struct UMicTestOptionsWidget_C_ExecuteUbergraph_MicTestOptionsWidget_Params
	{
	public:
		int32_t                                                    EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
